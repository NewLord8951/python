...
if len(quantity_sold) != 6 or any(len(day_quantity) != 5 for day_quantity
    ...
    for product_id in range(5):
        total_revenue = sum(quantity_sold[day]
                            [product_id] * price_per_item
                            [product_id] for day in range(6))
        revenue_per_product.append(total_revenue)
        print(f"Доход от продажи товара {product_id + 1}:\
               {total_revenue:.2f} руб.")

    daily_revenues = []
    for day in range(6):
        daily_revenue = sum(quantity_sold[day][product_id] * price_per_item
                            [product_id] for product_id in range(5))
    ...
    max_revenue_product = revenue_per_product.index(max
    ...
    print(f"Максимальный доход за день получен в день\
           {max_daily_revenue_day}.")

    a = int(input("Введите порог дохода (a): "))
    days_above_threshold = sum(1 for revenue in daily_revenues if revenue > a)
    print(f"Количество дней, когда доход превысил {a} руб.:\
